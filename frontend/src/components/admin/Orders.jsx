...
{filteredOrders.length === 0 ? (
      ...
      <table className="w-full min-w-full">
        <thead>
          <tr className="bg-[#f7e8dc] text-[#3f2e20]">
            <th className="py-3 px-3 md:px-4 text-left text-xs md:text-sm font-semibold">
              Order ID
            </th>
            <th className="py-3 px-3 md:px-4 text-left text-xs md:text-sm font-semibold">
              Customer
        ...
        <tbody>
          {filteredOrders.map((order) => (
            <tr
              key={order._id}
              className="border-b border-[#f1e5d8] hover:bg-[#fff7f1]"
            >
              <td className="py-3 px-3 md:px-4 text-xs md:text-sm font-semibold">
              ...
              <td className="py-3 px-3 md:px-4 text-xs md:text-sm font-bold">
  ...
  >
    ...
    <div className="md:hidden space-y-4 mt-2">
      {filteredOrders.map((order) => (
        <div
          key={order._id}
          className="bg-white rounded-xl shadow-md border border-[#f1e5d8] p-4"
        >
  ...
  <button
    onClick={() => viewOrderDetails(order)}
    className="flex-1 py-2 bg-blue-100 text-blue-700 rounded text-sm flex items-center justify-center gap-1"
  >
    <FaEye /> View
  </button>
  ...
  <button
    onClick={() => deleteOrder(order._id)}
    className="flex-1 py-2 bg-red-100 text-red-700 rounded text-sm flex items-center justify-center gap-1"
  >
    <FaTrash /> Delete
  </button>
</div>

{/* Status Dropdown (neeche alag row me, UX better) */}
<select
  value={order.orderStatus}
  onChange={(e) =>
    updateOrderStatus(order._id, e.target.value)
  }
  className="w-full mt-2 py-2 bg-gray-100 rounded text-sm"
>
  <option value="pending">Pending</option>
  <option value="confirmed">Confirmed</option>
  <option value="preparing">Preparing</option>
